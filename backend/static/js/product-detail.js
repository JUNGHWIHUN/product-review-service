document.addEventListener("DOMContentLoaded", function () {
    ...
    socket.onmessage = function (event) {
        ...
        if (data.status === "FAILURE") {
            resultBox.innerHTML = `<div class="ai-result-inner error"><p>${data.error || "AI 분석 중 오류가 발생했습니다."}</p></div>`;
            button.disabled = false;
            button.textContent = "비슷한 후기 보기";
            socket.close();
            return;
        ...
        if (data.status === "SUCCESS") {
            ...
            if (!result.similar_reviews || result.similar_reviews.length === 0) {
                resultBox.innerHTML = `<div class="ai-result-inner"><p>충분히 비슷한 후기를 찾지 못했어요.</p></div>`;
            } else {
                resultBox.innerHTML = `
                    <div class="ai-result-inner">
